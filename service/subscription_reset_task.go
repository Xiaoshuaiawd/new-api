...
func runSubscriptionQuotaResetOnce() {
	...
	for {
		n, err := model.ResetDueSubscriptionTokens(subscriptionResetBatchSize)
		if err != nil {
			logger.LogWarn(ctx, fmt.Sprintf("subscription token quota reset task failed: %v", err))
			return
		}
		if n == 0 {
			break
	...
	if common.DebugEnabled && (totalReset > 0 || totalTokenReset > 0 || totalExpired > 0) {
		logger.LogDebug(ctx, "subscription maintenance: user_reset_count=%d, token_reset_count=%d, expired_count=%d", totalReset, totalTokenReset, totalExpired)
	}
}
